    ...
    public class Model
    {
        ...
        public string CreateUser(string username, string password, string roles, string email)
        {
            ...
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(ConnStr("105","CustomerService")))
                {
                    cn.Open();
        ...
        public DataSet SearchMO(string source, string msisdn, string name)
        {
            ...
            if (name == "2345")
            {
                strSql = "SELECT CONVERT(VARCHAR(19),A.[datecreated]) AS [Date],RTRIM([msg_id]) AS [Message Id],RTRIM(A.[msisdn]) AS Msisdn,RTRIM(A.telconame) AS Telco,RTRIM(B.keyword) AS Keyword,RTRIM(A.[msg]) AS Message,RTRIM(A.[source]) AS Source FROM ";
                tblMO = "tblMO_tunetalk";
                database = "MOMT_NEW";
            }
            else if (name == "13999")
            {
                strSql = "SELECT CONVERT(VARCHAR(19),A.[datecreated]) AS [Date],[msg_id],[keyword],[msisdn],[msg],[telconame] FROM ";
                tblMO = "tblMO";
                database = "13999";
            }
            else if (name == "28282")
            ...
            {
                strSql = "SELECT [Id],[MoId],[DateCreated],[Msisdn],[KeywordName],[ContentType],[Message] FROM ";
                tblMO = "MO-ForestDetails";
                database = "SMSAdmin";
            }
            else
            {
            ...
            strSql += "[dbo].[" + tblMO + "] AS A ";

            if (name != "13999" && name != "20000")
                strSql += "INNER JOIN [smsadminv2].[dbo].[tblProvDet] AS B ON A.provid=B.provid ";
        ...
        public DataSet SearchMT(string source, string msisdn, string name)
        {
            ...
            {
                strSql = "SELECT CONVERT(VARCHAR(19),A.[datecreated]) AS [Date],[msg_id] AS [Message Id],[msisdn] AS [Msisdn],[telcoName] AS [Telco],msg AS Message,[Status],CAST(price/100 AS NVARCHAR) + '.00' AS [Price] FROM ";
                tblMT = "tblMT";
                database = "13999";
            }
            else if (name == "28282")
            {
                strSql = "SELECT CONVERT(VARCHAR(19),A.[datecreated]) AS [Date],A.[msg_id] AS [Message Id],RTRIM(A.[msisdn]) AS Msisdn,B.[Keyword],RTRIM(A.[statusmsg]) AS Message,CAST(A.price AS FLOAT)/100 AS [Price],RTRIM(A.[gateway]) AS Telco,RTRIM(A.[DNStatus]) AS DN FROM ";
                tblMT = "tblMT_umobile";
                database = "MOMT_NEW";
            }
            else if (name == "20088")
            ...
            {
                strSql = "SELECT [Id],[DateCreated],[MtId],[Msisdn],[Message],[KeywordName],[ContentType],[Price],[MtStatus] FROM ";
                tblMT = "MT-ForestDetails";
                database = "SMSAdmin";
            }
            else
            {
        ...
        public DataSet SearchMTGlobePH(string source, string msisdn)
        {
            ...
            string strSql = string.Empty;
            strSql = "SELECT [MtId] ,[DateCreated] ,[GameId],[Msisdn] ,[Message],[Keyword],[MTStatus],[Type],[Source] ,[Message2] FROM [GameWorld].[dbo].[tblMT] WHERE [Msisdn]='" + msisdn + "'";
        ...
        public DataSet SearchSubscriber(string source, string msisdn, string name)
        {
            ...
            if (name == "22355")
                strSql += ",(select CAST(SUM(a.price)/100 AS NVARCHAR) + '.00' from [MOMT_NEW].[dbo].[" + tblMT + "] AS A INNER JOIN [MOMT_NEW].[dbo].[DN_22355] AS B ON A.msg_id=B.msgid WHERE A.msisdn='" + msisdn + "' AND B.Status='Delivered' and A.provid=s.provid) as revenue ";
            else
                strSql += ",(select CAST(SUM(a.price)/100 AS NVARCHAR) + '.00' from [MOMT_NEW].[dbo].[" + tblMT + "] as a where a.msisdn='" + msisdn + "' and a.DNStatus='Delivered' and a.provid=s.provid) as revenue ";
        ...
        public void Unsubscribe(string source, int userid, string userlogin, string name)
        {
            ...
            if (name == "20088")
                strSQL = "UPDATE [tblSubscriber20088] SET [status]=1,[unsubby]=@unsubby,date_unsubs=GETDATE() WHERE userid=@userid and status=0;";
            else
        ...
        {
            Tools tl = new Tools();
            string strSQL = "UPDATE [tblSubscriber] SET [Status]=0,[Status2]=0,[UnsubBy]=@unsubby WHERE [SubsId]=@SubsId ";
            try
            {
                using (SqlConnection cn = new SqlConnection(ConnStr(source, "GameWorld")))
                {
        ...
        public void UnsubscribeAll(string source, string msisdn, string userlogin, string name)
        {
            ...
            if (name == "20088")
                strSQL = "UPDATE [tblSubscriber20088] SET [status]=1,[unsubby]=@unsubby,date_unsubs=GETDATE() WHERE msisdn=@msisdn and status=0;";
            else
        ...
        public string ConnStr(string source,string db)
        {
            ...
            //if (source == "102")
            //    con = "server=210.5.41.102;User id=dbuser;password=server9@9@;Max Pool Size=200;database=" + db;
            //else if (source == "103")
            //    con = "server=210.5.41.103;User id=dbuser;password=celcom2@13;Max Pool Size=200;database=" + db;
            //else if (source == "105")
            //    con = "server=localhost;User id=dbuser;password=server9@9@;Max Pool Size=200;database=" + db;
            //else if (source == "106")
            //    con = "server=210.5.41.106;User id=dbuser;password=celcom2@@8;Max Pool Size=200;database=" + db;
            //else if (source == "107")
            //    con = "server=210.5.41.107;User id=dbuser;password=celcom2@13;Max Pool Size=200;database=" + db;
            //return con;

            string con = string.Empty;
            if (source == "102")
                con = "server=192.168.28.39;User id=dbuser;password=server9@9@;Max Pool Size=200;database=" + db;
            else if (source == "103")
                con = "server=192.168.28.52;User id=dbuser;password=celcom2@13;Max Pool Size=200;database=" + db;
            else if (source == "105")
                con = "server=localhost;User id=dbuser;password=server9@9@;Max Pool Size=500;database=" + db;
            else if (source == "106")
                con = "server=192.168.28.149;User id=dbuser;password=celcom2@@8;Max Pool Size=200;database=" + db;
            else if (source == "107")
                con = "server=192.168.28.20;User id=dbuser;password=celcom2@13;Max Pool Size=200;database=" + db;
            else if (source == "101")
                con = "server=210.5.41.101;User id=dbussd;password=server9@9@;Max Pool Size=200;database=" + db;
            else if (source == "104")
                con = "server=192.168.28.142;User id=dbuser;password=celcom2@@8;Max Pool Size=200;database=" + db;
