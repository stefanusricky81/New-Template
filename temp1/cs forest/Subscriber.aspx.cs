        ...
        public DataSet SearchSubscriber(string source, string msisdn, string name)
        {
            ...
            if (useridentity[1] != "telco")
            {
                ...
                if (name == "22355")
                    ...
                    strSql += ",(select CAST(SUM(a.price)/100 AS NVARCHAR) + '.00' from [MOMTV2].[dbo].[" + tblMT + "] AS A WHERE A.msisdn='" + msisdn + "' AND A.DNStatus='Delivered' and A.provid=s.provid) as revenue ";
                else
